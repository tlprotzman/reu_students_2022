...
//  Global_FastSim();
  ...
  JetReco *calo_subtraction_jets = new JetReco();
  calo_subtraction_jets->add_input(new TowerJetInput(Jet::CEMC_TOWER));
  calo_subtraction_jets->add_input(new TowerJetInput(Jet::HCALIN_TOWER));
  calo_subtraction_jets->add_input(new TowerJetInput(Jet::HCALOUT_TOWER));
  calo_subtraction_jets->add_algo(new FastJetAlgo(Jet::ANTIKT, 0.2), "AntiKt_Tower_HIRecoSeedsRaw_r02");
  calo_subtraction_jets->set_algo_node("ANTIKT");
  calo_subtraction_jets->set_input_node("TOWER");
  calo_subtraction_jets->Verbosity(Input::VERBOSITY);
  se->registerSubsystem(calo_subtraction_jets);
